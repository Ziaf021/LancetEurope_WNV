...
rename_at(vars(as.vector(c(paste("prec_0", 1:4, sep=''), paste("min_temp_0", 1:4, sep=''), paste("mean_temp_0", 1:4, sep=''), paste("max_temp_0", 1:4, sep='')) ) ), 
 ...
 #   rename_at(vars(c("prec_01","prec_02", "prec_03", "prec_04")), ~ c("total_prec_Q01","total_prec_Q02", "total_prec_Q03", "total_prec_Q04"))%>%

LCD_predictors_NUTS3<- LCD_predictors_NUTS3_1950_21%>%
rbind(LCD_predictors_NUTS3_2022)%>%
left_join(read.csv(paste("/Users/ziafarooq/Old_Mac/Umu_PhD/WNF_project/WNV_Projections/West_Nile_Climate/Pop_and_GDP_proj_data_August_2022/population","ssp1soc","by_NUTS3_Regions_1950_2100.csv", sep='_'))%>%
  select(c(1,3,4, 6:7))%>%
  filter(str_length(NUTS_ID)>4),
  #dplyr::select(-c(2,4))
  by.x=c("NUTS_ID", "year", "CNTR_CODE"),
  by.y=c( "NUTS_ID", "year", "CNTR_CODE"), 
  all.x=T )%>%
  left_join(read.csv(paste("/Users/ziafarooq/Old_Mac/Umu_PhD/WNF_project/WNV_Projections/West_Nile_Climate/Pop_and_GDP_proj_data_August_2022/gdp_by_NUTS3_Regions_1950_2099.csv", sep='_'))%>%
  select(c(1,3,4, 6:7))%>%
  filter(str_length(NUTS_ID)>4),
  #dplyr::select(-c(2,4))
  by.x=c("NUTS_ID", "year", "CNTR_CODE"),
...
EU_NUTS3 <- get_eurostat_geospatial(resolution = 10,nuts_level = 3, 
 ...
 # left_join(WNV_Regs_already_infected,
 #           by.x= c("NUTS_ID"), by.y= c("NUTS_ID"),
 #           all.x=TRUE)
EU_NUTS3_area <-  EU_NUTS3[,c("NUTS_ID", "geometry")]%>%
  mutate(#.before = 44,
    area = st_area(geometry),
    areakm2 = as.numeric(units::set_units(area, km^2)))%>%
  dplyr::select(c(1,4))

LCD_predictors_NUTS3 <- LCD_predictors_NUTS3%>%
left_join(  EU_NUTS3_area%>%
              st_drop_geometry()%>%
               distinct(NUTS_ID,areakm2 )%>%
                filter(str_length(NUTS_ID)>4)%>%
                arrange(NUTS_ID),
              ...
              all.x=TRUE )%>%
    mutate(pop_density= population/areakm2)


#pop_data<- read.csv(paste("/Users/ziafarooq/Old_Mac/Umu_PhD/WNF_project/WNV_Projections/West_Nile_Climate/Pop_and_GDP_proj_data_August_2022/population","ssp1soc","by_NUTS3_Regions_1950_2100.csv", sep='_'))%>%
# select(c(1,3,4, 6:7))

#gdp_data<- read.csv(paste("/Users/ziafarooq/Old_Mac/Umu_PhD/WNF_project/WNV_Projections/West_Nile_Climate/Pop_and_GDP_proj_data_August_2022/gdp_by_NUTS3_Regions_1950_2099.csv", sep='_'))%>%
 # select(c(1,3,4, 6:7))

names(LCD_predictors_NUTS3)


 #pop_data<- read.csv(paste("/Users/ziafarooq/Old_Mac/Umu_PhD/WNF_project/WNV_Projections/West_Nile_Climate/Pop_and_GDP_proj_data_August_2022/population","ssp1soc","by_NUTS3_Regions_1950_2100.csv", sep='_'))

#write.csv(WNV_cases,file="WNV_cases.csv")
```
...
LCD_all_data <-  merge(LCD_predictors_NUTS3,df_WNVCases_2010_22,by.x=c("NUTS_ID", "year", "CNTR_CODE"),
                            by.y=c( "NUTS_ID", "year", "CNTR_CODE"), all.x=T)%>%
  mutate_at(c("wnf_sum"),~replace_na(.x, 0))%>% 
  mutate(wnf_case= ifelse(wnf_sum>0,"1","0"))%>%
  mutate(wnf_case = factor(wnf_case, 
                           levels = c("1", "0")))%>%na.omit()
...
LCD_test <- LCD_all_data%>% 
  filter (year %in% c(1950:2009, 2020:2022))%>%
    filter(str_length(NUTS_ID)>4)%>%
  dplyr::select(-c(1:4,40,42,44))
#dplyr::select(-c(1:4,40:44))
...
```{r}
df_wnv_pred_probs <- LCD_all_data%>%
  dplyr::select(c(1:4))%>%
...
write.csv(df_wnv_pred_probs[,c(2,1,3:5)], file = "LCDE_132_eur_NUTS3_1950_2022_2024_data.csv")

df_WNV_EU_CNTRY_1950to2022 <- df_wnv_pred_probs%>%group_by(year,CNTR_CODE)%>%
   summarize(mean_risk_prob= mean(pred_prob))#%>%

  write.csv(df_WNV_EU_CNTRY_1950to2022, file = "LCDE_132_eur_Country_1950_2022_2024_data.csv")

```

#Upload EU_regions Shape files
```{r}
shape_CE_Europe <- read_sf(paste(getwd(), '/Shapefiles/CE_Europe/CE_Europe.shp',sep = ''))%>%
   mutate(EU_reg= "CE_Europe")
 
 shape_North_Europe <- read_sf(paste(getwd(), '/Shapefiles/North_Europe/North_Europe.shp',sep = ''))%>%
   mutate(EU_reg= "North_Europe")
 
 shape_South_Europe <- read_sf(paste(getwd(), '/Shapefiles/South_Europe/South_Europe.shp',sep = ''))%>%
   mutate(EU_reg= "South_Europe")
 
 shape_West_Europe <- read_sf(paste(getwd(), '/Shapefiles/West_Europe/West_Europe.shp',sep = ''))%>%
   mutate(EU_reg= "West_Europe")
 shape_Europe_regions <- read_sf(paste(getwd(), '/Shapefiles/Europe_regions/Europe_regions.shp',sep = ''))%>%
   mutate(EU_reg= "Europe_regions")

```

## Create European Regions and Europe's CSV file 
```{r}
df_pred_probs_EU_RegWise <- df_wnv_pred_probs%>%filter(CNTR_CODE %in% shape_CE_Europe$CNTR_CODE )%>%
   mutate(EU_reg= "CE_Europe")%>%
   rbind(
     df_wnv_pred_probs%>%
       filter(CNTR_CODE %in% shape_North_Europe$CNTR_CODE )%>%
           mutate(EU_reg= "North_Europe")
     )%>%
   rbind(
     df_wnv_pred_probs%>%
       filter(CNTR_CODE %in% shape_South_Europe$CNTR_CODE | CNTR_CODE=="EL" )%>%
       #filter(CNTR_CODE=="EL")%>%
       mutate(EU_reg= "South_Europe")
   )%>%
   rbind(
     df_wnv_pred_probs%>%
       filter(CNTR_CODE %in% shape_West_Europe$CNTR_CODE | CNTR_CODE=="UK" )%>%
       #filter(CNTR_CODE=="UK")%>%
       mutate(EU_reg= "West_Europe")
   )%>%
   rbind(
...
write.csv(df_pred_probs_EU_RegWise%>%filter(EU_reg!="All_Europe")
             , file = "LCDE_132_eur_regions_1950_2022_2024_data.csv")

write.csv(df_pred_probs_EU_RegWise%>%filter(EU_reg=="All_Europe")
             , file = "LCDE_132_eur_eur_1950_2022_2024_data.csv")
...
## Estimate the probabilities
 ...
 df_PrcntChangeRisk <- 
 df_PrcntChangeRisk%>%mutate(across(2:16, round,2))
...
# Figures
## Figure West Nile virus -Main text LCDE 2024
```{r}

  df_WNVposRegs_EU_RegWise <- df_all_pred_resp%>%filter(CNTR_CODE %in% shape_CE_Europe$CNTR_CODE )%>%
   mutate(EU_reg= "CE_Europe")%>%
   rbind(
     df_all_pred_resp%>%
       filter(CNTR_CODE %in% shape_North_Europe$CNTR_CODE )%>%
  ...
  count()%>%
    ...
    mutate(EU_reg=
   #summarize(mean_risk_prob= mean(pred_prob))
  recode_factor(EU_reg , All_Europe= "All Europe", `CE_Europe` = "Eastern Europe", 
                                                 North_Europe = "Northern Europe", South_Europe= "Southern Europe", West_Europe = "Western Europe"))%>%
...
arrange(NUTS_ID,year) %>%
 ...
 ggplot(aes(year , 
   ...
   geom_smooth(
    ...
    geom_bar(df_WNVposRegs_EU_RegWise, mapping= aes(year , 
           # n/scal_factor, 
            n/1000, 
            fill=EU_reg
 ...
 scale_fill_manual(labels = c("Eastern Europe","Northern Europe","Southern Europe", "Western Europe",""),#,
                       values = c(LCD_figColors[1], LCD_figColors[2], LCD_figColors[3], LCD_figColors[4]),
                       guide = "legend" 
    )+
 ...
 #  scale_fill_ptol()+
  theme_bw()+
   ...
   scale_color_manual(labels = c("Eastern Europe","Northern Europe","Southern Europe", "Western Europe",""),#,
                       values = c(LCD_figColors[1], LCD_figColors[2], LCD_figColors[3], LCD_figColors[4]),
                       guide = "legend" 
    )
...
df_Mean_risk_EU_RegWise <- #df_pred_probs_EU_RegWise%>%
 ...
 df_Mean_risk_EU_RegWise$EU_reg <- recode_factor(df_Mean_risk_EU_RegWise$EU_reg , All_Europe= "All Europe", CE_Europe = "Central and Eastern Europe", 
                                                 North_Europe = "Northern Europe", South_Europe= "Southern Europe", West_Europe = "Western Europe")
 
 
 LCD_figColors <- c("#EE3377",	"#33BBEE" , "#EE7733", "#009988")
 
 
 df_Mean_risk_EU_RegWise$EU_reg <- recode_factor(df_Mean_risk_EU_RegWise$EU_reg , All_Europe= "All Europe", `Central and Eastern Europe` = "Eastern Europe", 
                                                 North_Europe = "Northern Europe", South_Europe= "Southern Europe", West_Europe = "Western Europe")
...
```{r}
  df_WNV_NUTS_1950_2022 <- df_wnv_pred_probs%>%group_by(year,NUTS_ID)%>%
   summarize(mean_risk_prob= mean(pred_prob))
 #df_wnv_proj_prob_maps <-   left_join(  europe_shp21[,c("NUTS_ID", "geometry")],df_WNV_NUTS_1950_2022, 
 #                             by.x= c("NUTS_ID"), by.y= c("NUTS_ID"),
#                              all.y=TRUE )

df_LCD_meanprbs_7tmprlgrps <-  df_WNV_NUTS_1950_2022%>%
  #st_drop_geometry()
   filter(year %in% (1951:1960))%>%
   group_by(NUTS_ID)%>%
   summarize(pp= mean(mean_risk_prob))%>%
   mutate(ProbYears=c("1950-1960"))%>%
  rbind(df_WNV_NUTS_1950_2022%>%
   filter(year %in% (1961:1970))%>%
   group_by(NUTS_ID)%>%
   summarize(pp = mean(mean_risk_prob))%>%
   mutate(ProbYears= c("1961-1970")) )%>%
  rbind(df_WNV_NUTS_1950_2022%>%
   filter(year %in% (1971:1980))%>%
   group_by(NUTS_ID)%>%
   summarize(pp = mean(mean_risk_prob))%>%
   mutate(ProbYears= c("1971-1980")) )%>%
  
  rbind(df_WNV_NUTS_1950_2022%>%
   filter(year %in% (1981:1990))%>%
   group_by(NUTS_ID)%>%
   summarize(pp = mean(mean_risk_prob))%>%
   mutate(ProbYears= c("1981-1990")) )%>%
  rbind(df_WNV_NUTS_1950_2022%>%
   filter(year %in% (1991:2000))%>%
   group_by(NUTS_ID)%>%
   summarize(pp = mean(mean_risk_prob))%>%
   mutate(ProbYears= c("1991-2000")) )%>%
 rbind(df_WNV_NUTS_1950_2022%>%
   filter(year %in% (2001:2010))%>%
   group_by(NUTS_ID)%>%
   summarize(pp = mean(mean_risk_prob))%>%
   mutate(ProbYears= c("2001-2010")) )%>%
   rbind(
     df_WNV_NUTS_1950_2022%>%
       filter(year %in% (2011:2022))%>%
       group_by(NUTS_ID)%>%
       summarize(pp = mean(mean_risk_prob))%>%
       mutate(ProbYears= c("2011-2022"))
     )
...
plot_decadal_change_risk<- 
  ...
  mutate(ProbYears= as.factor(ProbYears))%>%
...
df_means_pred_resp <- LCD_all_data%>%
  #dplyr::select(c(1:4,45))%>%
  filter(year %in% c(1950:2009, 2020:2022))%>%
  filter(str_length(NUTS_ID)>4)%>%
  mutate(pred_prob= LCD_xgb_pred)%>%
...
library(ggpubr)
library(survminer)
countries_with_WNV_transmission=  LCD_all_data%>%
  dplyr::select(c(1:4,45))%>%
  filter(wnf_case==1)%>%
  filter(year %in% c(2010:2019))
